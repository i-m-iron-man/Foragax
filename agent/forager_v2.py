...
class Forager(Agent):
    ...
    def create_agent(params: Params, unique_id: int, active_state: int, 
        ...
        def create_active_agent(key):
            ...
            Ang_acc = jax.random.uniform(keys[2], (1,), minval=-1.0, maxval=1.0)
            val = jax.random.uniform(keys[3], (1,), minval=5.0, maxval=10.0)
            rad = val
            time_above_repr_thr = jnp.zeros(1)
            time_below_death_thr = jnp.zeros(1)

            state_content = {'X_acc':X_acc, 'Y_acc':Y_acc, 'Ang_acc':Ang_acc, 
        ...
        def create_inactive_agent():
            state_content = {'X_acc': jnp.array([0.0]), 'Y_acc': jnp.array([0.0]), 'Ang_acc': jnp.array([0.0]),
    ...
    def step_agent(input:Signal, forager:Agent):
        def do_step(input, forager):
            ...
            Ang_acc = jnp.array([actions.content['actions'][2]])
            ...
            new_forager_state_content = { 'X_acc': X_acc, 'Y_acc': Y_acc, 'Ang_acc': Ang_acc, 
