...
def spawn_forager_pos(forager_set:Agent_Set, space:ContinuousSpace, key:jax.random.PRNGKey):
    ...
    def spawn_one_forager_pos(spawning_key, active_state, id, space):
        ...
        def spawn_active_forager_pos(spawning_key, id, space):
            key, *spawning_keys = jax.random.split(spawning_key, 7)
            ...
            Ang = jax.random.uniform(spawning_keys[4], (1,), minval = -jnp.pi, maxval = jnp.pi)
            Ang_vel = jax.random.uniform(spawning_keys[5], (1,), minval = -1.0, maxval = 1.0)
            id = id
            return forager_position(X, X_vel, Y, Y_vel, Ang, Ang_vel, id)
...
def update_forager_pos(forager_poses:forager_position, foragers:Forager, space:ContinuousSpace):
    ...
    def update_one_forager_pos(forager_pos, forager_state, forager_params, active_state, walls):
        ...
        def update_active_forager_pos(forager_pos, forager_state, forager_params, walls):
            ...
            Ang = forager_pos.Ang + forager_params.content['dt']*forager_pos.Ang_vel
            ...
            Ang_vel = forager_pos.Ang_vel + forager_params.content['dt']*forager_state.content['Ang_acc']
            ...
            return forager_pos.replace(X = X, Y = Y, X_vel = X_vel, Y_vel = Y_vel, Ang = Ang, Ang_vel = Ang_vel)
...
def remove_forager_pos(forager_poses, indexes_to_remove, num_to_remove):
    def remove_pos(idx, forager_poses__indexes_to_remove):
        ...
        Ang_vel = forager_poses.Ang_vel.at[indexes_to_remove[idx]].set(jnp.array([0.0]))
        
        new_forager_poses = forager_poses.replace(X = X, X_vel = X_vel, Y = Y, Y_vel = Y_vel, Ang = Ang, Ang_vel = Ang_vel)
...
def add_forager_pos(forager_poses, num_agents_add, add_param, space, key):
    def add_one_pos(idx, forager_poses__add_param__space__key):
        ...
        key, *noise_keys = jax.random.split(key, 7)
        ...
        Y_vel = forager_poses.X_vel.at[forager_to_add_idx].set(jax.random.uniform(noise_keys[3], (1,), minval = -1.0, maxval = 1.0))

        Ang = forager_poses.Ang.at[forager_to_add_idx].set(forager_to_copy.Ang + jax.random.uniform(noise_keys[4], (1,), minval = -0.1, maxval = 0.1))
        Ang_vel = forager_poses.Ang_vel.at[forager_to_add_idx].set(forager_to_copy.Ang_vel + jax.random.uniform(noise_keys[5], (1,), minval = -0.1, maxval = 0.1))
...
def Neuroevolution(foragers:Forager, forager_poses: forager_position, resources:Resource, resource_poses: resource_position, space:ContinuousSpace, 
    ...
    select_params = Params(content={'energy_threshold': 1.5})
    ...
    select_params = Params(content={'time_threshold': 2.5})
...
class Foraging_World:
    ...
    def __init__(self, foragers_params:dict, resources_params:dict, space_params:dict, sim_steps: jnp.int32, max_record_steps: jnp.int16, key=jax.random.PRNGKey(2)):
        ...
        self.checkpoint_manager = orbax.checkpoint.CheckpointManager('/home/siddhart/source/Foragax_v2/world/orbax',
...
foragers_params = {'total_num': 1000, 'active_num': 330, 'type': 1, 'num_neurons': 40, 'num_obs': 27, 
                   'num_actions': 3, 'dt': 0.1, 'repr_thresh':3.5, 'action_scale': 1, 'deterministic': False, 'cooling_period': 2.7}
...
f_world = Foraging_World(foragers_params, resources_params, space_params, sim_steps=1000000, max_record_steps=1000, key=jax.random.PRNGKey(2))

f_world.run(print_freq=20, record_freq=5000, record_start_step=20000)